...
2. Implement clock stretching (TODO #2)
3. Run a multi-byte transaction (TODO #3)
...
def encode_i2c_transaction(device_addr, register_addr, data_bytes, read=False):
    ...
    # --- Address byte + R/W bit (Always Write first for register addressing) ---
    # 7-bit address, MSB first, then R/W bit (0=write)
    addr_bits = [(device_addr >> (6 - i)) & 1 for i in range(7)]
    addr_byte_write = addr_bits + [0] # Always 0 for the register write phase
    sda_seq, scl_seq = bits_to_waveform(addr_byte_write)
    ...
    if read:
        ...
        # SDA goes HIGH, then LOW while SCL is HIGH. For simplicity, just use the START seq.
        segments.append(("R-START", [1, 1, 0, 0], [1, 1, 1, 0], "REPEATED START"))

        # --- Address byte + R/W bit (Read) ---
        addr_byte_read = addr_bits + [1] # 1=read
        sda_seq, scl_seq = bits_to_waveform(addr_byte_read)
        segments.append(("ADDR+R", sda_seq, scl_seq, f"ADDR=0x{device_addr:02X} R"))
...
def decode_i2c_segments(segments):
    ...
    for label, sda_bits, scl_bits, annotation in segments:
        if label in ("START", "STOP", "R-START"):
...
def simulate_nack():
    print("\n--- Simulating NACK (Device Not Found) ---")
    segments = []
    device_addr = 0x77 # Non-existent device
    ...
    # ADDR+RW
    addr_bits = [(device_addr >> (6 - i)) & 1 for i in range(7)]
    addr_byte = addr_bits + [0] # Write
    sda_seq, scl_seq = bits_to_waveform(addr_byte)
    segments.append(("ADDR+RW", sda_seq, scl_seq, f"ADDR=0x{device_addr:02X} W"))
    
    # NACK from missing slave
    ack_sda, ack_scl = generate_ack(ack=False)
    segments.append(("NACK", ack_sda, ack_scl, "NACK"))
...
def simulate_clock_stretching():
    print("\n--- Simulating Clock Stretching ---")
    # Simulate stretch by adding extra LOWs to SCL during ACK
    ack_sda = [0] * (SAMPLES_PER_BIT + 10)  # Extended SDA low
    ack_scl = [0] * (SAMPLES_PER_BIT // 4 + 10) + \
              [1] * (SAMPLES_PER_BIT // 2) + \
              [0] * (SAMPLES_PER_BIT // 4)
    
    segments = [
        ("START", [1, 1, 0, 0], [1, 1, 1, 0], "START"),
        ("ADDR+RW", *bits_to_waveform([(0x48 >> (6-i))&1 for i in range(7)] + [0]), "ADDR=0x48 W"),
        ("STRETCH", ack_sda, ack_scl, "STRETCH_ACK"),
        ("STOP", [0, 0, 1, 1], [0, 1, 1, 1], "STOP")
    ]
    return segments
...
def simulate_mpu6050():
    print("\n--- MPU6050 Multi-byte Read Simulation ---")
    DEVICE_ADDR = 0x68
    REGISTER = 0x3B
    DATA = [0x03, 0xE8, 0xFF, 0x01] # Just first 4 bytes to keep it simple
    return encode_i2c_transaction(DEVICE_ADDR, REGISTER, DATA, read=True)
...
def main():
    ...
    # Run TODOs
    nack_segments = simulate_nack()
    stretch_segments = simulate_clock_stretching()
    mpu_segments = simulate_mpu6050()
